...
Usage:
------
This program requires: FileManager.py, Topology.py, Additionals.py, Molecule.py,
H_bond, and PlotManger.py.

The purpose of this program is to quantify and plot the hydrogen bond interaction
between two different species of molecules.
The two different species of molecules are referred as: molec0 and molec1.

This program requires:
a)  The GRO files to calculate the hydrogen bonds. The grofiles should be frames from a
    simulation.
...
b)  The two itp files (one for each of the two molecule species).
...
a)  The hydrogen from a molecules species has to be chemically bonded to another
    atom in the same molecule. This is represented by X-H.
    Where H is the hydrogen atom, X is the atom chemically bonded to the hydrogen atom,
    and "-" represents the chemical bond.

b)  The third atom has to come the other molecule species, and it is represented by Y.
    Therefore, the notation X-H---Y is the Y atom interacting with the hydrogen from
    the other molecule species, and "---" represent the dipole-dipole interaction.

c)  The Y atom should be within a maximum distance threshold from the Hydrogen atom only.

d)  The configuration X-H---Y should comply with the cosine rule.
    In this cosine rule we need all there distances (XtoH, XtoY, and HtoY), and the
    angle that Y forms with X-H should have a minimum threshold in order to be considered
    a hydrogen bond.

e)  Hydrogen bonds cannot take place if Y atom is an Alkali (i.e. sodium, potassium, or calcium)
    and a hydrogen atom as well.
...
b)  Please use python h_bonds_calc.py --help for usage information.
    Please be sure to read carefully all the help info and comments to clear any
    doubts.

c)  The hydrogen bond considerations in this program are supported in the publication:
...
Gromacs must be installed in order to determine the density profile.
